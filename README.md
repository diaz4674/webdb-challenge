...
1. Explain the difference between `RDBMS` and `SQL`.
      Relational Database Management System is the database itself of where data is being pulled, updated, etc. Structured Query Language is what we use to communicate with the RDBS.

1. Why do tables need a `primary key`?

    - Tables need a primary key to uniquely identify each piece of data.
...
1. What do we need in order to have a _many to many_ relationship between two tables.
  ...
  -We need to create another table that references each table with foreign keys.
